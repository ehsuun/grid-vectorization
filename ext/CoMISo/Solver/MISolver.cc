    ...
    bool       _fixed_order )
{
  ...
  // modification by Tibor Stanko : show number of variables
  std::cout << "# int  vars : " << _to_round.size() << "\n" //
            << "# real vars : " << _x.size()-_to_round.size() << std::endl;
    ...
    bool       _fixed_order )
{
  ...
  // modification by Tibor Stanko : progress bar
  const int N_bars = 30;
  int i_bar = 0;
  auto print_progress = [&i_bar, &N_bars](int i,  // index of current element (0-based)
                                          int N){ // number of elements
    if (i == -1) {
      std::cout << "[" << std::string(N_bars, ' ') << "]";
    } else {
      if ((float)i >= (float)(i_bar * (N - 1)) / (float)(N_bars - 1)) {
        ++i_bar;
        std::cout << std::string(N_bars + 2, '\b') //
                  << "["                      //
                  << std::string(i_bar, '*')      //
                  << std::string(N_bars - i_bar, ' ')  //
                  << "]"                      //
                  << std::flush;
      }
      if (i == N - 1)
  ...
  print_progress(-1, to_round.size());

  // loop until solution computed
  for(unsigned int i=0; i<to_round.size(); ++i)
  {
    print_progress((int)i, to_round.size());
